...
#set this if you are using nengo DL
if batch_processing:
...
def create_model(seed=None):
    ...
    with model:
        ...
        if nengo_gui_on:
            with nengo.Simulator(network=model, progress_bar=False) as sim_first:
        ...
        if(True):
            ...
            if nengo_gui_on:
                with nengo.Simulator(network=model, progress_bar=False) as sim_second:
        ...
        if not(nengo_gui_on):
            ...
            if store_memory:
                model.p_mem_first_raw=nengo.Probe(memory_first.neurons, 'output', synapse=0.05)
                model.p_mem_second_raw=nengo.Probe(memory_second.neurons, 'output', synapse=0.05)
...
def plot_sim_1(sp_1,sp_2,res_1,res_2,cal_1,cal_2, mem_1, mem_2):
    ...
    with plt.rc_context():
        ...
        plt.savefig(figure_name + '_exp2_3.eps', format='eps', dpi=1000)
        plt.show()
...
if nengo_gui_on:
    ...
    if sim_to_run == 2:
            ...
            for batch in range(n_batches):
                ...
                if store_decisions:
                    for in_batch_trial in range(minibatch_size):
                        ...
                        np.savetxt(data_path+"Performance/exp2/"+sim_no+"_Diff_Theta_%i_subj_%i_trial_%i_probe1.csv" % (anglediff, subj+1, trial),
                            sim.data[model.p_dec_first][in_batch_trial,int((1800)/res):int((2100)/res),:], delimiter=",")

                        np.savetxt(data_path+"Performance/exp2/"+sim_no+"_Diff_Theta_%i_subj_%i_trial_%i_probe2.csv" % (anglediff, subj+1, trial),
                            sim.data[model.p_dec_second][in_batch_trial,int((4300)/res):int((4600)/res),:], delimiter=",")

                        # os.chmod(data_path+"Performance/exp2/"+sim_no+"_Diff_Theta_%i_subj_%i_trial_%i_probe1.csv" % (anglediff, subj+1, trial) , 0o777)
                        # os.chmod(data_path+"Performance/exp2/"+sim_no+"_Diff_Theta_%i_subj_%i_trial_%i_probe2.csv" % (anglediff, subj+1, trial) , 0o777)
                ...
                if store_memory:
                    ...
                    if (batch+1)%(n_batches/split)==0:
                        ...
                        np.save(data_path+"Decoding/exp2/subj_%i_mem_neuron_data_first_%i.npy" % (subj+1, split_index), neuro_mem_first)
                        np.save(data_path+"Decoding/exp2/subj_%i_mem_neuron_data_second_%i.npy" % (subj+1, split_index), neuro_mem_second)
                        np.save(data_path+"Decoding/exp2/subj_%i_initial_angles_%i.npy" % (subj+1, split_index), initialangles)

                        # os.chmod(data_path+"Decoding/exp2/subj_%i_mem_neuron_data_first_%i.npy" % (subj+1, split_index), 0o777)
                        # os.chmod(data_path+"Decoding/exp2/subj_%i_mem_neuron_data_second_%i.npy" % (subj+1, split_index), 0o777)
                        # os.chmod(data_path+"Decoding/exp2/subj_%i_initial_angles_%i.npy" % (subj+1, split_index), 0o777)
        ...
        print('1,50x0,030n0,010 90ms+51ms')
    ...
    if sim_to_run == 3: #check for decision times
        ...
        for trial in range(ntrials):
            ...
            first = np.add(first, sim.data[model.p_dec_first][(1800):(2100),:])
            second = np.add(second, sim.data[model.p_dec_second][(4300):(4600),:])

            sim.close()

        first = first/ntrials
        second = second/ntrials
        ...
        with plt.rc_context():
            ...
            plot_first=axes[0]
            plot_first.set_title("Tested early")
            plot_first.plot(first)
            plot_second=axes[1]
            plot_second.set_title("Tested late")
            plot_second.plot(second)

            name = "decisions2final"
