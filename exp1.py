...
def get_data(inputs_cued, inputs_uncued, inputs_reactivate, initialangles, n_trials, probelist, res):
    start = timer()
...
def create_model(seed=None):
    ...
    with model:
        ...
        if nengo_gui_on:
            with nengo.Simulator(network=model, progress_bar=False) as sim_cued:
        ...
        if uncued:
            ...
            if nengo_gui_on:
                with nengo.Simulator(network=model, progress_bar=False) as sim_uncued:
        ...
        if not(nengo_gui_on):
            ...
            if store_memory:
                model.p_mem_cued_raw=nengo.Probe(memory_cued.neurons, 'output', synapse=0.05)
                model.p_mem_uncued_raw=nengo.Probe(memory_uncued.neurons, 'output', synapse=0.05)
...
def plot_sim_1(sp_c,sp_u,res_c,res_u,cal_c,cal_u, mem_cued, mem_uncued):
    ...
    with plt.rc_context():
        ...
        plot_mc.plot(sim.trange(),(mem_cued));
        plot_mc.set_ylabel("Cosine similarity")
        plot_mc.set_xticks(np.arange(2.15,2.45,0.05))
        ...
        plot_mu.plot(sim.trange(),(mem_uncued));
        plot_mu.set_xticks(np.arange(2.15,2.45,0.05))
...
if nengo_gui_on:
    ...
    if sim_to_run == 1:
        ...
        for run in range(ntrials):
            ...
            sim = nengo_dl.Simulator(network=model, seed=run, progress_bar=False, device=device) # device=device
        ...
        sim = nengo_dl.Simulator(network=model, seed=0, progress_bar=False, device=device)  #device=device
    ...
    if sim_to_run == 2:
        ...
        for subj in range(n_subj):
            ...
            for batch in range(n_batches):
                ...
                get_data(input_data_cued, input_data_uncued, input_data_reactivate, data_initialangles, minibatch_size ,probelist,res)
                ...
                if store_decisions:
                    for in_batch_trial  in range(minibatch_size):
                        ...
                        anglediff = probelist[in_batch_trial%14]

                        trial = batch * minibatch_size + in_batch_trial + 1

                        np.savetxt(data_path+"Performance/exp1/"+sim_no+"_Diff_Theta_%i_subj_%i_trial_%i.csv" % (anglediff, subj+1, trial),
                            sim.data[model.p_dec_cued][in_batch_trial,int((2650)/res):int((2950)/res),:], delimiter=",")


                        # os.chmod(data_path+"Performance/exp2/"+sim_no+"_Diff_Theta_%i_subj_%i_trial_%i.csv" % (anglediff, subj+1, trial) , 0o777)
                ...
                if store_memory:
                    ...
                    if (batch+1)%(n_batches/split)==0:
                        ...
                        np.save(data_path+"Decoding/exp1/subj_%i_mem_neuron_data_cued_%i.npy" % (subj+1, split_index), neuro_mem_cued)
                        np.save(data_path+"Decoding/exp1/subj_%i_mem_neuron_data_uncued_%i.npy" % (subj+1, split_index), neuro_mem_uncued)
                        np.save(data_path+"Decoding/exp1/subj_%i_initial_angles_%i.npy" % (subj+1, split_index), initialangles)

                        # os.chmod(data_path+"Decoding/exp2/subj_"+str(subj+1)+"_mem_neuron_data_cued.npy", 0o777)
                        # os.chmod(data_path+"Decoding/exp2/subj_"+str(subj+1)+"_mem_neuron_data_uncued.npy", 0o777)
                        # os.chmod(data_path+"Decoding/exp2/subj_"+str(subj+1)+"_initial_angles.npy", 0o777)
            ...
            # np.save(data_path+"Decoding/exp2/subj_"+str(subj+1)+"_mem_neuron_data_cued.npy", neuro_mem_cued)
            # np.save(data_path+"Decoding/exp2/subj_"+str(subj+1)+"_mem_neuron_data_uncued.npy", neuro_mem_uncued)
            # np.save(data_path+"Decoding/exp2/subj_"+str(subj+1)+"_initial_angles.npy", initialangles)
            #
            # os.chmod(data_path+"Decoding/exp2/subj_"+str(subj+1)+"_mem_neuron_data_cued.npy", 0o777)
            # os.chmod(data_path+"Decoding/exp2/subj_"+str(subj+1)+"_mem_neuron_data_uncued.npy", 0o777)
            # os.chmod(data_path+"Decoding/exp2/subj_"+str(subj+1)+"_initial_angles.npy", 0o777)
    ...
    if sim_to_run == 3: #check for decision times

        store_decisions = True

        load_gabors_svd =False

        phase = 0
        memory_item_cued = 0 + 90 + phase
        probe_cued = 40 + 90 + phase
        memory_item_uncued = 0 + 90 + phase
        probe_uncued = 40 + 90 + phase
        ...
        for trial in range(ntrials):
            model = create_model()
            sim = nengo_dl.Simulator(network=model, seed=0, device=device, progress_bar=False)

            sim.run(3)

            cued = np.add(cued, sim.data[model.p_dec_cued][(2650):(2950),:])
        ...
        with plt.rc_context():
            ...
            plot_cued=axes[0]
            plot_cued.set_title("Cued")
            plot_cued.plot(cued)

            name = "decisions_1.50x0.30n0.10"
